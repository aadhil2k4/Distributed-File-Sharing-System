...
class ChordNode:
    ...
    def upload_file(self, filename, recv_ip_port, replicate):
        ...
        s_list = [1, 1 if replicate else -1, filename]
    ...
    def download_file(self, filename):
        ...
        try:
            ...
            if file_data == b"NotFound":
                ...
                print("Receiving file:", filename)
                self.receive_file(client_socket, filename)  # Fix here
        except socket.error:
    ...
    def send_file(self, connection, filename):
        ...
        try:
            with open(filename, 'rb') as file:
                ...
                if file_size == 0:
                    print("Warning: File is empty, not sending any data.")
                    return  # Prevent sending an empty file

                # Send file data
                connection.sendall(data)
                print(f"File {filename} sent successfully, Size: {file_size} bytes")
    ...
    def receive_file(self, connection, filename):
        ...
        try:
            ...
            if not file_size_bytes:
                print("Error: No file size received")
                return
            file_size = int.from_bytes(file_size_bytes, byteorder='big')
            print(f"Expected file size: {file_size} bytes")

            if file_size == 0:
                print("Warning: Received an empty file. Skipping write operation.")
            ...
            # Verify if the full file is received
            if received_size != file_size:
                print("Error: File received is incomplete")
                return

            # Write file only if data is received correctly
            with open(filename, 'wb') as file:
                file.write(data)

            print(f"File {filename} received successfully, Size: {received_size} bytes")
